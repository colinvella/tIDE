...
namespace TileMapEditor.Controls
{
	...
	public class CustomTabControl : TabControl
	{
		...
		protected override CreateParams CreateParams
        {
            ...
            get
            {
                ...
                if (this.RightToLeftLayout)
                    cp.ExStyle = cp.ExStyle | TabControlNativeMethods.WS_EX_LAYOUTRTL | TabControlNativeMethods.WS_EX_NOINHERITLAYOUT;
                return cp;
            }
        }
		...
		public int ActiveIndex {
			get {
				TabControlNativeMethods.TCHITTESTINFO hitTestInfo = new TabControlNativeMethods.TCHITTESTINFO(this.PointToClient(Control.MousePosition));
				int index = TabControlNativeMethods.SendMessage(this.Handle, TabControlNativeMethods.TCM_HITTEST, IntPtr.Zero, TabControlNativeMethods.ToIntPtr(hitTestInfo)).ToInt32();
				if (index == -1){
					return -1;
				} else {
		...
		protected override void OnFontChanged(EventArgs e){
			...
			TabControlNativeMethods.SendMessage(this.Handle, TabControlNativeMethods.WM_SETFONT, hFont, (IntPtr)(-1));
			TabControlNativeMethods.SendMessage(this.Handle, TabControlNativeMethods.WM_FONTCHANGE, IntPtr.Zero, IntPtr.Zero);
			this.UpdateStyles();
			if (this.Visible){
				this.Invalidate();
			}
	...
	protected override bool ProcessMnemonic(char charCode){
		...
		protected override void WndProc(ref Message m){
			...
			switch (m.Msg) {
				case TabControlNativeMethods.WM_HSCROLL:
					
					//	Raise the scroll event when the scroller is scrolled
					base.WndProc(ref m);
					this.OnHScroll(new ScrollEventArgs(((ScrollEventType)TabControlNativeMethods.LoWord(m.WParam)),_oldValue, TabControlNativeMethods.HiWord(m.WParam), ScrollOrientation.HorizontalScroll));
					break;
		...
		private Rectangle GetTabTextRect(int index){
			...
			using (GraphicsPath path = this._StyleProvider.GetTabBorder(index)){
				...
				if (this.ImageList != null && (this.TabPages[index].ImageIndex > -1 
					...
					Rectangle imageRect = this.GetTabImageRect(index);
					if ((this._StyleProvider.ImageAlign & TabControlNativeMethods.AnyLeftAlign) != ((ContentAlignment) 0)) {
						if (this.Alignment <= TabAlignment.Bottom) {
					...
					} else if ((this._StyleProvider.ImageAlign & TabControlNativeMethods.AnyCenterAlign) != ((ContentAlignment) 0)) {
						//	If there is a closer allow for it
						if (this._StyleProvider.ShowTabCloser) {
							Rectangle closerRect = this.GetTabCloserRect(index);
							if (this.Alignment <= TabAlignment.Bottom) {
